...
RSpec.describe Cart do
  ...
  describe '.discounted_total' do
    before :each do
      ...
      @dollar_coupon = create(:coupon, value: 10)
      @percentage_coupon = create(:percentage_coupon, value: 50)
      @cart = Cart.new(nil)
      @item_1 = create(:item, price: 5.0, user: @merchant)
      @item_2 = create(:item, price: 10.0, user: @merchant)
      @item_2 = create(:item, price: 20.0, user: @other_merchant)
    end
    ...
    it 'returns a discounted total for a percentage based coupon' do
      ...
      expect(@cart.discounted_total(@percentage_coupon)).to eq(5.0)

      @cart.add_item(@item_2.id.to_s)

      expect(@cart.discounted_total(@percentage_coupon)).to eq((@item_1.price + @item_2.price) / @percentage_coupon.value)
    end
  end
end
